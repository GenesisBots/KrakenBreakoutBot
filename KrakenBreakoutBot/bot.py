#!/usr/bin/env python
# KrakenBreakoutBot — standalone open-source edition
...
ORDERS_LOG_FILE = Path(__file__).resolve().parent / "kraken_breakout_orders_log.jsonl"

COINGECKO_BASE = "https://api.coingecko.com/api/v3"

# MODE: "paper" (default) or "live"
MODE = os.getenv("KRAKEN_BO_MODE", "paper").lower()
...
class SymbolState:
    def __init__(self, symbol: str, donchian_period: int, rsi_period: int, vol_period: int):
        self.symbol = symbol
    ...
    def add_tick(self, ts: float, price: float, volume: float) -> None:
        self.last_price = price
        self.last_volume = volume
        self.prices.append((ts, price))
        self.volumes.append(volume)

        max_len = max(self.donchian_period, self.rsi_period, self.vol_period) * 3
        while len(self.prices) > max_len:
            self.prices.popleft()
        while len(self.volumes) > max_len:
            self.volumes.popleft()

    def closes(self) -> List[float]:
        return [p for _, p in self.prices]

    def donchian(self) -> Optional[tuple[float, float]]:
        closes = self.closes()
        if len(closes) < self.donchian_period:
            return None
        window = closes[-self.donchian_period:]
    ...
    def volume_sma(self) -> float:
        if len(self.volumes) < self.vol_period:
            return 0.0
        window = list(self.volumes)[-self.vol_period:]
        return sum(window) / len(window)


def mc_breakout_confidence(st: SymbolState, num_paths: int, horizon_sec: int) -> float:
    closes = st.closes()
    if len(closes) < 10 or st.last_price is None:
        return 0.5
    ...
    _, donchian_high = st.donchian() or (None, None)
    ...
    for _ in range(num_paths):
        ...
        if price > donchian_high:
            breakout_count += 1
    return breakout_count / num_paths
...
def compute_breakout_score(
    ...
    if don is None:
        return -999.0
    don_low, don_high = don
    price = st.last_price

    # require price above Donchian high
    if price <= don_high:
        return -999.0
    ...
    vol_sma = st.volume_sma()
    if st.last_volume <= vol_sma:
        return -999.0

    if mc_conf < mc_min_conf:
        return -999.0

    dist = (price - don_high) / don_high * 100.0
...
def maybe_trade(
    ...
    for sym, st in symbol_states.items():
        mc_conf = mc_breakout_confidence(st, mc_paths, mc_horizon_sec)
        s = compute_breakout_score(
            st,
            rsi_max_long,
            vol_min_pct,
            vol_max_pct,
    ...
    don_low, don_high = st.donchian() or (0.0, 0.0)

    reason = (
        f"LONG {best_sym} breakout price={price:.2f} "
        f"don_high={don_high:.2f} rsi={st.rsi():.1f} "
        f"vol={st.volatility_pct():.3f}% vol_sma={st.volume_sma():.3f} "
        f"mc_conf={best_mc_conf:.3f} score={best_score:.3f}"
    )
                ...
                async for raw in ws:
                    ...
                    if isinstance(msg, dict) and msg.get("channel") == "ticker":
                        ...
                        for tick in data:
                            ...
                            vol_str = tick.get("volume", {}).get("today")
                            ...
                            try:
                                ...
                                continue
                            try:
                                volume = float(vol_str) if vol_str is not None else 0.0
                            except Exception:
                                volume = 0.0
...
async def main() -> None:
    ...
    rsi_max_long = float(cfg.get("rsi_max_long", 65.0))

    vol_min_pct = float(cfg.get("vol_min_pct", 0.10))
    vol_max_pct = float(cfg.get("vol_max_pct", 4.0))

    take_profit_pct = float(cfg.get("take_profit_pct", 1.8))
    hard_sl_pct = float(cfg.get("hard_sl_pct", 0.9))
    trail_start_pct = float(cfg.get("trail_start_pct", 1.2))
    trail_step_pct = float(cfg.get("trail_step_pct", 0.4))
    ...
    symbol_states: Dict[str, SymbolState] = {}
    for sym in symbols:
        symbol_states[sym] = SymbolState(sym, donchian_period, rsi_period, vol_period)
